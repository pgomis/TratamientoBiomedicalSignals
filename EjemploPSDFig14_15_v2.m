% Ejemplo señal sinusoidal de 40 Hz de 3 segundos
Fo = 40;
To = 1/Fo;
fs = 1000;
Ts = 1/fs; 
...
E = trapz(t, x.^2) % 7200 W·s
E2 = Ts*sum(x.^2)
Pm=1/N*sum(x.^2);  % Potencia media
X=fft(x); % equivalente a X=fft(x,N);
Ener= (Ts/N)*sum(abs(X).^2)   %   Ener =   6.000  [W-s]
f = (0:N-1)*fs/N;
figure(1)
% Figura 14
...
ylabel('Amplitud')
if mod(N,2)==1           % se evalua la paridad de N
...
Pm=fs*sum(Pxx_u)/N  % Potencia media
